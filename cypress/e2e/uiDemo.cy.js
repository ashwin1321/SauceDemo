...
import topTwoItemss from "../fixtures/topTwoItems.json"
...
describe("UI demo Add to Cart", () => {
    ...
    it("Login and add to cart", () => {
        ...
        let topTwoItems = topTwoItemss.items;
        ...
        topTwoItems.forEach((item, index) => {
        ...
        cy.get(inventory.carticon).should("have.text", topTwoItems.length);
        ...
        cart.getCartLength().then(length => {
            expect(length).to.eq(topTwoItems.length);
        ...
        topTwoItems.forEach((item, index) => {
            cart.getItemName(index).then(name => {
                ...
                const exists = topTwoItems.some(item => item.name === name);
